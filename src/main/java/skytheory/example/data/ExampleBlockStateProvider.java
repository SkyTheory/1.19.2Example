...
public class ExampleBlockStateProvider extends BlockStateProvider {
	...
	public ExampleBlockStateProvider(DataGenerator generator, String modid, ExistingFileHelper existingFileHelper) {
	...
	protected void registerStatesAndModels() {
		 ...
		 * 何も描画しないモデルを作成する
		 */
		this.blockEntity(BlockInit.EXAMPLE_ENTITY_BLOCK.get(), new ResourceLocation("minecraft", "block/stone"));
	}
	...
	public BlockModelBuilder createChildModel(Block block, ResourceLocation parent) {
		BlockModelBuilder model = this.modelProvider.withExistingParent(this.getRegistryKey(block).getPath(), parent).renderType("translucent");
	...
	public void blockEntity(Block block, ResourceLocation particle) {
		ConfiguredModel model = new ConfiguredModel(this.models().getBuilder(getRegistryKey(block).getPath()).texture("particle", particle));
		this.getVariantBuilder(block).partialState().setModels(model);
	}
